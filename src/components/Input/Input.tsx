...
export default function Input<TFieldValues extends FieldValues = FieldValues>({
  ...
  classNameInput = 'form__input flex-1 h-full text-base md:text-lg font-medium placeholder:text-slate-300 ',
  ...
  return (
    <div className={'relative ' + className}>
      {icon && <div className='absolute left-3 top-1/2 transform -translate-y-1/2'>{icon}</div>} {/* Add icon here */}
      <input className={`${classNameInput} ${icon ? 'pl-10' : ''}`} {...registerResult} {...rest} type={handleType()} />
